...
let portListsAnalog numOfPorts hostID portType =
    if numOfPorts < 1 
    then []
    else
        [0..(numOfPorts-1)]
...
let initPortOrientation (comp: Component) =
    ...
    let inputMaps : PortMaps =
        addPortToMaps Left {Order=defaultportOrder; Orientation=Map.empty} comp.OutputPorts[0].Id

    let res = 
        addPortToMaps Right {Order=defaultportOrder; Orientation=Map.empty} comp.OutputPorts[1].Id
    res
...
let makeComponent (pos: XYPos) (comptype: ComponentType) (id:string) (label:string) : Component =
    ...
    let makeComponent' (n, nout, h, w) label : Component=
        let inputPorts = portLists n id PortType.Output
...
let init () = 
    { 
        ...
        Ports = Map.empty ; OutputPortsConnected= Set.empty
    }, Cmd.none
...
let inline getTwoPortLocations (model: Model) (inputPortId: OutputPortId ) (outputPortId: OutputPortId) =
    (getOutputPortLocation None model inputPortId, getOutputPortLocation None model outputPortId)
